...
namespace iCTF_Website.Controllers {
    ...
    public class FlagsController : Controller {
        ...
        public async Task<IActionResult> SubmitAsync(string apiKey, [FromBody] SubmitData data)
        {
            ...
            if (challenge == null)
            {
                return Json(new { Success = false, Error = "Your answer is incorrect" });
            } 
            else if (challenge.State == 3) 
            {
                return Json(new { Success = false, Error = "You are trying to submit an answer for an archived challenge" });
            }
